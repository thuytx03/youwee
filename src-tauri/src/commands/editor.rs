...
pub struct EditorVideoMetadata {
    pub path: String,
    pub filename: String,
    pub duration: f64,
    pub width: i32,
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct EditorFFmpegCommandResult {
    pub command: String,
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct EditorProcessingProgress {
    pub job_id: String,
    pub percent: f64,
    pub frame: i64,
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct EditorShotDetectionResult {
...
fn try_build_subtitle_command(
    user_prompt: &str,
    input_path: &str,
    metadata: &EditorVideoMetadata,
    subtitle_attachments: &[EditorAttachment],
    output_dir: Option<&str>,
) -> Result<Option<EditorFFmpegCommandResult>, String> {
    ...
    Ok(Some(EditorFFmpegCommandResult {
...
async fn try_build_merge_command(
    ...
    input_path: &str,
    metadata: &EditorVideoMetadata,
    video_attachments: &[EditorAttachment],
    output_dir: Option<&str>,
) -> Result<Option<EditorFFmpegCommandResult>, String> {
    ...
    for path in &ordered_paths {
        let info = editor_get_video_metadata(app.clone(), path.clone()).await?;
...
#[tauri::command]
pub async fn editor_generate_command(
    app: AppHandle,
    input_path: String,
    user_prompt: String,
    ...
    metadata: EditorVideoMetadata,
    attachments: Option<Vec<EditorAttachment>>,
    output_dir: Option<String>,
) -> Result<EditorFFmpegCommandResult, String> {
    ...
    let video_attachments: Vec<EditorAttachment> = attachments
        .iter()
        .filter(|a| a.kind == "video")
        .cloned()
        .collect();
    let subtitle_attachments: Vec<EditorAttachment> = attachments
        .iter()
...
#[tauri::command]
pub async fn editor_generate_quick_action_command(
    input_path: String,
    task_type: String,
