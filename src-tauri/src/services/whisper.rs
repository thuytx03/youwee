    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            WhisperError::UnsupportedFormat(fmt) => {
                write!(
                    f,
                    "Unsupported audio format: {}. Use mp3, mp4, m4a, wav, or webm.",
                    fmt
                )
            }
...
fn convert_verbose_json_to_subtitle(
    ...
    let verbose: WhisperVerboseResponse =
        serde_json::from_str(response_text).map_err(|e| WhisperError::ParseError(e.to_string()))?;
...
pub async fn extract_audio_for_whisper(
    ...
    if !output.status.success() {
        ...
        return Err(WhisperError::FfmpegError(format!(
            "FFmpeg failed: {}",
            stderr
        )));
    }
...
pub async fn get_audio_duration(
    ...
    duration_str.parse::<f64>().map_err(|e| {
        WhisperError::ParseError(format!("Invalid duration '{}': {}", duration_str, e))
    })
}
...
mod tests {
    ...
    fn test_response_format_display() {
        ...
        assert_eq!(
            WhisperResponseFormat::VerboseJson.to_string(),
            "verbose_json"
        );
    }
