...
### 新增
- **插件商店** - 新增独立的设置菜单，可从 GitHub 实时目录发现并安装经过校验的插件包
- **文件名元数据** - 新增下载设置，可将上传日期、观看次数、上传者、时长、分辨率或视频 ID 等所选视频信息添加到文件名前缀
- **罗马尼亚语界面** - 新增应用界面的罗马尼亚语翻译
- **乌兹别克语界面** - 新增应用界面的乌兹别克语翻译
- **土耳其语界面** - 新增应用界面的土耳其语翻译

### 修复
- **AppImage 图标** - 更新 Tauri 构建工具和兼容依赖，让 Linux AppImage 保留正确的桌面图标元数据

## [0.20.0] - 2026-07-08

### 新增
- **日语界面** - 新增应用界面的日语翻译
- **西班牙语界面** - 新增应用界面的西班牙语翻译
- **Universal 编码选择** - 为 Universal 下载新增视频编码选择
- **长视频 AI Summary** - 新增格式选项和连贯的分段摘要，让长视频的详细摘要保留更多上下文
...
### 新增
- **资料库集合** - 新增自动创建资料库集合的选项，适用于展开的播放列表、频道下载和拆分出的章节文件
- **资料库删除行为** - 新增设置，可选择删除资料库项目时是否同时删除媒体文件
- **帧率** - 在下载、Universal 和频道中新增优先下载 30 FPS 的选项，并提供清晰的画质提示
- **已下载视频记忆** - 新增设置，可在下载、Universal、Gallery 和频道中检测资料库已有视频，并选择询问、跳过或允许重复
- **拆分工具** - 新增可选项目操作，可使用 FFmpeg 将已完成的媒体文件拆分为命名片段

### 变更
- **资料库历史** - 移除最大历史条目设置，让资料库保留已下载视频记录，直到用户自行删除
- **资料库摘要** - 优化资料库项目操作，并将完整 AI 摘要移入带字号控制的独立阅读弹窗
- **队列下载文件夹** - 更改全局下载文件夹时，询问是否将新文件夹应用到队列项目

### 修复
- **扩展冷启动链接** - 修复浏览器扩展下载链接在应用未运行时只打开 Youwee 但未添加视频的问题
- **AI Summary 编号** - 修复主题列表在项目之间有空行时界面编号总是从 1 重新开始的问题
- **频道获取控制** - 新增频道视频获取停止操作，并避免打开刚浏览过的已关注频道时重复获取
- **重复项弹窗缩略图** - 修复已下载视频弹窗在资料库项目已有缩略图时仍显示占位图的问题
- **设置图标** - 修复 Font Awesome 图标在图标 CSS 不可用时过大并破坏设置布局的问题

## [0.18.0] - 2026-06-29

### 新增
- **队列文件编号** - 新增选项，可为普通队列下载文件添加队列顺序前缀
- **队列下载文件夹** - 新增队列项目操作，可为单个下载选择保存文件夹
- **yt-dlp 高级选项** - 新增经过验证的高级选项，供需要细调 yt-dlp 下载命令的用户使用
- **播放列表文件编号** - 新增下载设置，可在播放列表下载文件名前添加播放列表顺序
- **内嵌章节拆分** - 新增下载设置，可将视频内嵌章节拆分为独立文件，并可选择为章节文件编号
- **扩展 AI 摘要** - 在浏览器扩展中新增摘要按钮，可将 YouTube 视频直接打开到 AI Summary
- **AI 摘要 Token 上限** - 在设置中新增可选输入框，用于自定义生成摘要时的最大输出 Token 数
- **Cookie 跳过规则** - 新增基于标签的规则，可对匹配的网站跳过浏览器 Cookie 或 Cookie 文件认证，并默认包含 Facebook Reels

### 变更
- **扩展界面** - 优化浏览器扩展弹窗和悬浮菜单，使其更简洁现代，并与音乐播放器风格更一致

### 修复
- **插件恢复下载** - 修复 `download.failed` 插件成功恢复文件后，原队列项目仍显示失败的问题
- **插件重试工作流** - 修复重试失败下载时未使用新分配的插件工作流的问题
- **扩展悬浮按钮** - 修复浏览器扩展在已打开的标签页中安装或重新加载后悬浮按钮不显示或崩溃的问题
- **扩展冷启动下载** - 修复桌面应用尚未运行时，在浏览器扩展点击 `Download now` 只打开 Youwee 但未添加视频的问题
- **AI 摘要视频信息** - 修复字幕可用但 yt-dlp 无法选择视频格式时，AI Summary 卡在获取视频信息的问题
- **AI 摘要长度** - 移除默认硬编码的摘要输出 Token 限制，除非用户自定义数值，否则使用模型提供商的默认值
- **过长视频标题** - 修复使用过长标题作为文件名时下载失败的问题

## [0.17.2] - 2026-06-17

### 变更
- **界面字体和图标** - 将应用字体和图标打包进应用，让界面不依赖外部 CDN 也能保持一致显示
- **插件详情布局** - 将设置中已导入插件的详情改为标签页展示，包含信息、请求的权限以及运行时与兼容性
- **频道加载更多** - 将频道浏览页和已关注频道详情页的加载更多按钮改为与 YouTube 关键词搜索一致的浮动样式

### 修复
- **音乐播放器显示状态** - 修复音乐播放器收起后仍留下不可见覆盖层的问题，避免遮挡下方按钮并在点击被遮挡区域时重新开始播放
- **频道完成状态** - 修复重新打开应用后已关注频道视频丢失完成状态的问题，现在会持久化手动频道下载完成状态，并按精确视频 ID 与仍存在的历史文件恢复状态
- **重复关注频道** - 再次关注相同 URL 时返回已有的已关注频道记录，避免视频同步到不存在的频道 ID

## [0.17.1] - 2026-06-11

### 新增
- **即将开始直播的定时下载** - 在队列项目中新增内联操作，可在 YouTube 直播尚未开始时为后续重试设置定时
- **数据导出 YouTube 关键词来源** - 新增 YouTube 关键词来源，可在 Data Export 中导出搜索结果
- **CLI 输出文件夹** - 新增 `--output` / `-o`，让每个通过 CLI 加入队列的下载使用自己的绝对输出文件夹

### 变更
- **定时控件** - 优化定时弹窗和已定时状态，加入更快的预设、更清晰的预览文本和更紧凑的倒计时显示
...
- **队列状态标签页** - 为 YouTube 和 Universal 队列新增紧凑的状态标签页，可按下载状态筛选视频
...
### 变更
- **插件引导与权限流程** - 优化了插件导入、权限审批、workspace 指南和配置界面，使插件安装、工作流分配以及在 Settings 中阅读 Guide 更清晰
- **提升插件作者灵活性** - 更新 workspace 与 SDK 集成，支持更灵活的 Lucide 插件图标，并为插件作者提供更简洁的 Deno 本地测试流程

### 修复
- **插件运行日志与输出摘要** - 减少重复的插件日志噪音，不再保存插件结果的原始协议输出，并在 post-processing 日志中更清楚地显示后续步骤会使用的输出文件路径
- **插件 Guide 与多语言文档显示** - 修复 Guide 弹窗内容溢出的问题，并保留已安装插件的 `README.<locale>.md` 文件
- **插件配置界面打磨** - 改进 multi-select 控件、校验触发时机，以及导入/启用插件时的提示，使其与应用整体界面更一致
