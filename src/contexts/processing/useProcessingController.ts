...
import {
  ...
  deleteProcessingJob,
  deleteProcessingPreset,
  executeFfmpegBatch,
  executeFfmpegCommand,
  generateAudioPreview,
  generateProcessingCommand,
  generateQuickActionCommand,
  generateVideoPreview,
  generateVideoThumbnail,
  getProcessingAttachmentInfo,
  getProcessingHistory,
  getProcessingPresets,
  getVideoMetadata,
...
export function useProcessingController(): ProcessingContextValue {
  // Video state
  ...
  useEffect(() => {
    const setupListener = async () => {
      unlistenRef.current = await onProcessingProgress((event) => {
  ...
  const loadHistory = useCallback(async () => {
    try {
      const jobs = await getProcessingHistory(50);
      setHistory(jobs);
    } catch (error) {
          ...
          if (shouldGeneratePreview) {
            ...
            try {
              ...
              const [previewPath, audioPath, thumbPath] = await Promise.all([
                generateVideoPreview(path, metadata.video_codec, metadata.format),
                metadata.has_audio ? generateAudioPreview(path) : Promise.resolve(null),
                generateVideoThumbnail(path),
              ]);

              // Load thumbnail as blob URL (fallback for when <video> fails)
              const fileData = await readBinaryFile(thumbPath);
              const blob = new Blob([toBlobBuffer(fileData)], { type: 'image/jpeg' });
            ...
            setIsGeneratingPreview(true);
            try {
              const thumbPath = await generateVideoThumbnail(path);
              const fileData = await readBinaryFile(thumbPath);
              const blob = new Blob([toBlobBuffer(fileData)], { type: 'image/jpeg' });
              const thumbUrl = URL.createObjectURL(blob);
              setThumbnailSrc(thumbUrl);
  ...
  const selectOutputDirectory = useCallback(async () => {
    try {
      const selected = await pickProcessingOutputDirectory(
        outputDirectory || (videoPath ? getDirectoryFromPath(videoPath) : undefined),
      );
  ...
  const attachImages = useCallback(async (paths: string[]) => {
    ...
    for (const path of paths) {
      try {
        const info = await getProcessingAttachmentInfo(path);
        let previewUrl: string | undefined;

        if (info.kind === 'image') {
          // Read image as blob for preview
          const fileData = await readBinaryFile(path);
          const blob = new Blob([toBlobBuffer(fileData)]);
  ...
  const sendMessage = useCallback(
    async (content: string) => {
      ...
      try {
        ...
        const result = await generateProcessingCommand({
          inputPath: videoPath,
          userPrompt: content,
  ...
  const generateCommand = useCallback(
    async (taskType: ProcessingTaskType, options?: Record<string, unknown>) => {
      ...
      try {
        const result = await generateQuickActionCommand({
          inputPath: videoPath,
          taskType,
          options: options ?? {},
          timelineStart: selection?.start ?? null,
  ...
  const cancelProcessing = useCallback(async () => {
    if (currentJobId) {
      try {
        await cancelFfmpeg(currentJobId);
        await updateProcessingJob({
          id: currentJobId,
          status: 'cancelled',
          progress: 0,
  ...
  const openOutputFolder = useCallback(async () => {
    if (completedOutputPath) {
      try {
        await revealOutputInFolder(completedOutputPath);
      } catch (error) {
  ...
  const loadPresets = useCallback(async () => {
    try {
      const savedPresets = await getProcessingPresets();
  ...
  const deletePreset = useCallback(async (id: string) => {
    try {
      await deleteProcessingPreset(id);
